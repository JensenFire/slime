...
class UpdateWeightFromRDMA(UpdateWeightFromRemote):
    ...
    def _create_inference_replica(
        self, model_path: str, pp_shard: int, target_rank: int, target_tp: int,
        dp_rank: int,
        dp_size: int,
        attn_tp_rank: int,
        attn_tp_size: int,
        ...
        with MockSglangDistributedContext(tp_size=target_tp, tp_rank=target_rank, 
                dp_rank=dp_rank,
                dp_size=dp_size,
...
class MockSglangDistributedContext:
    ...
    def __enter__(self):
        ...
        self._patches = [
            ...
            patch("sglang.srt.distributed.parallel_state.get_moe_expert_parallel_world_size", return_value=self.ep_size),
            patch("sglang.srt.distributed.parallel_state.get_moe_tensor_parallel_rank", return_value=self.moe_tp_rank),
            patch("sglang.srt.distributed.parallel_state.get_moe_tensor_parallel_world_size", return_value=self.moe_tp_size),
            ...
            patch("sglang.srt.layers.moe.token_dispatcher.standard.get_moe_expert_parallel_rank", return_value=self.ep_rank),
            patch("sglang.srt.layers.moe.token_dispatcher.standard.get_moe_expert_parallel_world_size", return_value=self.ep_size),
